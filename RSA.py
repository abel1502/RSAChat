...
class PublicKey:
    ...
    def encrypt(self, msg):
        ...
        #if blockSize >= self.n:
        #    raise Exception("<ERROR in RSA.PublicKey.encrypt(): Each block must be less than N>")
        if type(msg) == bytes:
    ...
    def _encryptBlock(self, msg):
        ...
        if msg >= self.n:
            raise Exception("<ERROR in RSA.PublicKey._encryptBlock(): Message must be less than N>")
    ...
    def load(keyStr):
        ...
        keyStr = keyStr.strip()
        if keyStr.count("#") != 4:
            raise Exception("<ERROR in RSA.PublicKey.load(): keyStr is not a valid AbelRSA Public Key>")
        keyStr = keyStr.strip('#')
...
class PrivateKey:
    ...
    def _decryptBlock(self, msg):
        ...
        if type(msg) == bytes:
            msg = int.from_bytes(msg, "big")
        if msg >= self.n:
            raise Exception("<ERROR in RSA.PrivateKey._decryptBlock(): Message must be less than N>")
    ...
    def decrypt(self, msg):
        ...
        #blockSize = 256 ** blockLen
        #if blockSize >= self.n:
        #    raise Exception("<ERROR in RSA.PrivateKey.decrypt(): Each block must be less than N>")
        if type(msg) == bytes:
    ...
    def sign(self, msg):
        ...
        if type(msg) == bytes:
            msg = int.from_bytes(msg, "big")
        if msg >= self.n:
            raise Exception("<ERROR in RSA.PrivateKey.sign(): Message must be less than N>")
        return pow(msg, self.d, self.n)
    ...
    def load(keyStr):
        ...
        keyStr = keyStr.strip()
        if keyStr.count("#") != 5:
            raise Exception("<ERROR in RSA.PrivateKey.load(): keyStr is not a valid AbelRSA Private Key>")
        keyStr = keyStr.strip('#')
